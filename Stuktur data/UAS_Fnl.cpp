...
void displayCategoryRelationships() {
    if (categoryGraph.edges == nullptr) {
        cout << "Tidak ada hubungan kategori." << endl;
        return;
    }

    cout << "Hubungan Kategori:\n";
    for (Edge* edge = categoryGraph.edges; edge != nullptr; edge = edge->next) {
        cout << edge->source->name << " -> " << edge->destination->name << endl;
    }
}

void setCategoryRelationship() {
    char sourceCategoryName[MAX_NAME_LENGTH], destinationCategoryName[MAX_NAME_LENGTH];

    cout << "Masukkan nama kategori sumber: ";
    cin.ignore();
    cin.getline(sourceCategoryName, MAX_NAME_LENGTH);

    cout << "Masukkan nama kategori tujuan: ";
    cin.getline(destinationCategoryName, MAX_NAME_LENGTH);

    Category* sourceCategory = findCategory(root, sourceCategoryName);
    Category* destinationCategory = findCategory(root, destinationCategoryName);

    if (sourceCategory && destinationCategory) {
        categoryGraph.addRelationship(sourceCategory, destinationCategory);
        cout << "Hubungan kategori berhasil ditambahkan." << endl;
    } else {
        cout << "Salah satu atau kedua kategori tidak ditemukan." << endl;
    }
}
                    ...
                    switch (choice) {
                        ...
                        case 5:
                            ...
                            switch (choice) {
                                case 1:
                                    system("cls");
                                    setCategoryRelationship();
                                    break;
                                case 2:
                                    system("cls");
                                    displayCategoryRelationships();
                                    break;
                                default:
